...
extern uint32_t can_1_recv;
extern uint32_t can_2_recv;
...
unsigned int  waitForLoop = 0;

char CAN_ids[256] = {1,0};
char CAN_selfID;
...
void CAN_IRQHandler (void)  {
  ...
  if (icr & (1 << 0)) {                          /* CAN Controller #1 meassage is received */
    ...
    LPC_CAN1->CMR = (1 << 2);                    /* Release receive buffer */
		...
		if(CAN_RxMsg[0].id == 0 && CAN_RxMsg[0].data[0]== 0 && waitForLoop == 0)
		{
			for (i = 0; i < CAN_RxMsg[0].data[1]; i++){
				//CAN_wrFilter(1, i, STANDARD_FORMAT);
				CAN_ids[i] = 1;
			}
			
			i = CAN_RxMsg[0].data[1];
			while(CAN_ids[i] != 0){i++;};
			//GUI_Text(0,270, (uint8_t*)"CAN", Black, Yellow);
			CAN_TxMsg[1].id = 0;
			CAN_TxMsg[1].data[0] = 0;
			CAN_TxMsg[1].data[1] = i;
			CAN_TxMsg[1].len = 2;
			CAN_ids[i] = 1;
			CAN_wrMsg(2, &CAN_TxMsg[1]);
		}
		else if (CAN_RxMsg[0].id == 0 && CAN_RxMsg[0].data[0]== 0 && waitForLoop == 1)
		{
			CAN_selfID = CAN_RxMsg[0].data[1];
			//CAN_wrFilter(1, CAN_selfID, STANDARD_FORMAT);
			for (i = 0; i <= CAN_selfID; i++){
				CAN_ids[i] = 1;
			}
			waitForLoop = 0;
		}
		else if (CAN_RxMsg[0].id == CAN_TxMsg[0].id)
		{
			CAN_ids[CAN_RxMsg[0].id] = 0;
		}
		else
		{
			if (CAN_RxMsg[0].id != CAN_selfID){
				/*CAN_TxMsg[1].id = CAN_RxMsg[0].id;
				CAN_TxMsg[1].format = CAN_RxMsg[0].format;
				CAN_TxMsg[1].len = CAN_RxMsg[0].len;
				CAN_TxMsg[1].data[0] = CAN_RxMsg[0].data[0];
				CAN_TxMsg[1].data[1] = CAN_RxMsg[0].data[1];
				CAN_TxMsg[1].data[2] = CAN_RxMsg[0].data[2];
				CAN_TxMsg[1].data[3] = CAN_RxMsg[0].data[3];
				CAN_TxMsg[1].data[4] = CAN_RxMsg[0].data[4];
				CAN_TxMsg[1].data[5] = CAN_RxMsg[0].data[5];
				CAN_TxMsg[1].data[6] = CAN_RxMsg[0].data[6];
				CAN_TxMsg[1].data[7] = CAN_RxMsg[0].data[7];
				CAN_wrMsg(2, &CAN_TxMsg[1]);*/
				CAN_wrMsg(2, &CAN_RxMsg[0]);
				can_1_recv = can_1_recv == 1 ? 0 : 1;
			}
		}
